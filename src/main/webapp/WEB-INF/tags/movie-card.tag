...
<%@ attribute name="movie" required="true"
	type="com.java.spring.movie.model.entity.Movie"%>
...
<a href="${root}/movie-detail?id=${movie.id}"
	...
	<div class="relative aspect-[2/3] rounded-lg overflow-hidden shadow-lg">
		...
		<div
			...
			<div class="absolute bottom-0 left-0 right-0 p-4">
				...
				<div class="flex items-center gap-2">
					<%-- Play Button --%>
					<div
						onclick="window.location.href='${root}/movie/watch?id=${movie.id}'; event.stopPropagation();"
						class="flex-1 flex items-center justify-center gap-1 bg-red-600 hover:bg-red-700 text-white py-2 rounded text-sm font-medium transition-colors">
					...
					<div
						...
						class="watchlist-btn w-9 h-9 flex items-center justify-center rounded transition-colors backdrop-blur-sm 
           						 ${movie.inWatchlist ? 'bg-green-600' : 'bg-gray-700/80 hover:bg-gray-600'}">
		...
		<div
			class="absolute top-2 right-2 bg-black/70 backdrop-blur-md px-2 py-1 rounded flex items-center gap-1 border border-white/10">
